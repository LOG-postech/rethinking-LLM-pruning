...
def prepare_calibration_input(model, dataloader, device, config):
    ...
    inps = torch.zeros((recon_nsamples, config.seqlen, model.config.hidden_size), dtype=dtype, device='cpu')
...
def prune_model(config, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    dataloader, _ = get_loaders("gen_add", nsamples=config.nsamples,seed=config.seed,seqlen=config.seqlen,tokenizer=tokenizer, self_nsamples=config.self_nsamples)
    with torch.no_grad():
        inps, outs, attention_mask, position_ids = prepare_calibration_input(model, dataloader, device, config)
    layers = model.model.layers
    ...
    for i in range(update_start, update_start + update_round):
        ...
        if config.use_cr:
            with torch.no_grad():
                ...
                for j in range(0, recon_nsamples, config.infer_batch_size):
                    dense_outs[j:j+config.infer_batch_size] = obtain_output(dense_layers[start_idx:end_idx + 1], dense_inps[j:j+config.infer_batch_size].to(device), attention_mask=attention_mask.expand(config.infer_batch_size, -1, -1, -1), position_ids=position_ids).to('cpu')
                for j in range(start_idx, end_idx + 1):
        ...
        if config.use_cr:
            if i < 0:
                ...
                recon_error = val(layer[0:1], inps, dense_outs, config, device, attention_mask, position_ids)
